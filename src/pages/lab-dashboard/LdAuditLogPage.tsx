...
import { useLdAuditLog } from "@/hooks/useLdAuditLog";
import { motion } from "framer-motion";

const eventTypeColors: Record<string, string> = {
  case_created: "bg-blue-100 text-blue-700",
  case_updated: "bg-amber-100 text-amber-700",
  case_completed: "bg-emerald-100 text-emerald-700",
  invoice_created: "bg-green-100 text-green-700",
  inventory_updated: "bg-purple-100 text-purple-700",
  client_added: "bg-cyan-100 text-cyan-700",
};

export default function LdAuditLogPage() {
  const [eventType, setEventType] = useState("");
  const [search, setSearch] = useState("");
  const { data: logs = [] } = useLdAuditLog({ eventType: eventType || undefined });
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Audit Log" description="Track all lab activity and changes">
        <Badge variant="outline" className="gap-1"><Shield className="h-3 w-3" /> Activity Tracking</Badge>
      </PageHeader>
