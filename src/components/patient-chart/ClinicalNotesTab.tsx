...
export function ClinicalNotesTab({ patientId, clinicalNotes, canEdit, userId }: ClinicalNotesTabProps) {
    ...
    if (noteId && attachments.length > 0) {
      for (const file of attachments) {
        await uploadImage.mutateAsync({
          ...
          description: `Attached to clinical note`,
          userId,
  ...
  const getNoteImages = (noteId: string) => allImages.filter((img: any) => img.clinical_note_id === noteId);

  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-sm font-medium">Clinical Notes (SOAP)</h3>
        {canEdit && (
      ...
      {clinicalNotes.length === 0 ? (
        <Card><CardContent className="py-10 text-center text-sm text-muted-foreground">No clinical notes recorded yet.</CardContent></Card>
      ) : (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editingNote ? "Edit Clinical Note" : "Add Clinical Note"}</DialogTitle></DialogHeader>
